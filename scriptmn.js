function _defineProperty(obj, key, value) {
	if (key in obj) {
		...
		enumerable: true, 
		configurable: true, 
		writable: true });} else {obj[key] = value;}return obj;} // clearing the console (just a CodePen thing)
...
function setupAnimation(model)
{
  ...
  gsap.set(plane.position, { x: 80, y: -32, z: 50 });
...
//come on stage
  ...
  tl.to(plane.position, { x: -10, z: 50, ease: 'power1.in' }, delay);

  delay += sectionDuration;

//fly across and first turn manuver upon entering ground section
  tl.to(plane.rotation, { x: tau * -.05, y: tau * 0, z: -tau * 0.05, ease: 'power1.inOut' }, delay);
  tl.to(plane.position, { x: -80, y: 0, z: -60, ease: 'power1.inOut' }, delay);

  delay += sectionDuration;

///swing back across and tilt some
  tl.to(plane.rotation, { x: tau * 0, y:  tau * 0, z: tau * 0, ease: 'power3.inOut' }, delay);
  tl.to(plane.position, { x: -20, y: 20, z: -60, ease: 'power2.inOut' }, delay);

  delay += sectionDuration;

//swing back across, tilt some and grow
  tl.to(plane.rotation, { x: tau * .2, y: 0, z: -tau * 0.1, ease: 'power3.inOut' }, delay);
  tl.to(plane.position, { x: 50, y: 20, z: -30, ease: 'power2.inOut' }, delay);

  delay += sectionDuration;

//dip, twist, flatten back out and grow upon entering blueprint section through Length area
  tl.to(plane.rotation, { x: 0, z: 0, y: tau * .25 }, delay);
  tl.to(plane.position, { x: 0, y: -10, z: 150 }, delay);

  delay += sectionDuration;
  delay += sectionDuration;

// transition for wingspan area
  tl.to(plane.rotation, { x: tau * 0.25, y: tau * .5, z: 0, ease: 'power4.inOut' }, delay);
  tl.to(plane.position, { z: 30, ease: 'power4.inOut' }, delay);

  delay += sectionDuration;
// transition for left falange
  tl.to(plane.rotation, { x: tau * 0.25, y: tau * .5, z: 0, ease: 'power4.inOut' }, delay);
  tl.to(plane.position, { z: 160, x: 30, ease: 'power4.inOut' }, delay);

  delay += sectionDuration;
// transition for engine
...
// transition out of blueprint
  ...
  delay += sectionDuration;
//transition straight out of page
  tl.to(plane.rotation, { duration: sectionDuration, x: -tau * 0.05, y: tau, z: -tau * 0.1, ease: 'none' }, delay);
  tl.to(plane.position, { duration: sectionDuration, x: 0, y: 0, z: 220, ease: 'power1.in' }, delay);
